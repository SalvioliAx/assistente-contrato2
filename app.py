...
from firebase_admin import credentials, firestore, storageAdd commentMore actions
...
# --- FUNÇÕES DE GERENCIAMENTO DE COLEÇÕES (MODIFICADAS PARA FIREBASE) ---
# --- FUNÇÕES DE GERENCIAMENTO DE COLEÇÕES ---

# Lista coleções salvas no Firestore
def listar_colecoes_salvas():
    if not db: return []
    try:
@@ -122,7 +113,6 @@ def listar_colecoes_salvas():
        st.error(f"Erro ao listar coleções do Firebase: {e}")
        return []

# Salva a coleção atual no Firestore e Firebase Storage
...
@@ -132,48 +122,38 @@ def salvar_colecao_atual(nome_colecao, vector_store_atual, nomes_arquivos_atuais
        return False
    ...
    with st.spinner(f"A salvar coleção '{nome_colecao}' no Firebase..."):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                # Salvar o manifesto com os nomes dos arquivos
                manifest_path = os.path.join(temp_dir, "manifest.json")
                with open(manifest_path, "w") as f:
                    json.dump(nomes_arquivos_atuais, f)

                # Criar um arquivo zip com o conteúdo do diretório
                ...
                # Fazer upload do zip para o Firebase Storage
...
def carregar_colecao(nome_colecao, _embeddings_obj):
    if not db or not BUCKET_NAME:
@@ -182,7 +162,6 @@ def carregar_colecao(nome_colecao, _embeddings_obj):

    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            # Obter metadados do Firestore
            doc_ref = db.collection('ia_collections').document(nome_colecao)
            doc = doc_ref.get()
            if not doc.exists:
@@ -197,20 +176,17 @@ def carregar_colecao(nome_colecao, _embeddings_obj):
                st.error(f"Metadados incompletos para a coleção '{nome_colecao}'.")
                return None, None

            # Baixar o arquivo zip do Firebase Storage
            bucket = storage.bucket(BUCKET_NAME)
...
@@ -267,17 +236,17 @@ def obter_vector_store_de_uploads(lista_arquivos_pdf_upload, _embeddings_obj):
...
@@ -290,98 +259,80 @@ def obter_vector_store_de_uploads(lista_arquivos_pdf_upload, _embeddings_obj):
...
def extrair_dados_dos_contratos(_vector_store: Optional[FAISS], _nomes_arquivos: list) -> list:
    ...
    resultados_finais = []
@@ -405,10 +356,9 @@ def extrair_dados_dos_contratos(_vector_store: Optional[FAISS], _nomes_arquivos:
...
@@ -418,8 +368,7 @@ def extrair_dados_dos_contratos(_vector_store: Optional[FAISS], _nomes_arquivos:
...
@@ -439,17 +388,10 @@ def extrair_dados_dos_contratos(_vector_store: Optional[FAISS], _nomes_arquivos:
...
@@ -458,330 +400,44 @@ def extrair_dados_dos_contratos(_vector_store: Optional[FAISS], _nomes_arquivos:
...
@@ -791,6 +447,7 @@ def formatar_chat_para_markdown(mensagens_chat):
...
@@ -803,438 +460,101 @@ def formatar_chat_para_markdown(mensagens_chat):
                    ...
                    with st.expander("Ver Fontes Utilizadas"):
                        for doc_fonte in message["sources"]:
                            ...
                            if sentenca_chave and sentenca_chave in texto_fonte: 
                                ...
                                st.markdown(f"{partes[0]}**{sentenca_chave}**{partes[1] if len(partes) > 1 else ''}", unsafe_allow_html=True)
                            else: 
            ...
            if lista_nomes_arquivos_resumo:
                ...
                if arquivo_selecionado_nome_resumo and st.button("✍️ Gerar Resumo Executivo", key="btn_resumo_tab_v3", use_container_width=True):
                    ...
                    if arquivo_obj_selecionado:
                        arquivo_bytes = arquivo_obj_selecionado.getvalue()
        ...
        if arquivos_pdf_originais_global:
            ...
            if 'eventos_contratuais_df' in st.session_state and st.session_state.eventos_contratuais_df is not None:
                ...
                if not df_display_eventos.empty:
                    df_display_eventos['Data Formatada'] = pd.NaT
                    ...
                    if 'Data Objeto' in df_display_eventos.columns and pd.api.types.is_datetime64_any_dtype(df_display_eventos['Data Objeto']) and df_display_eventos['Data Objeto'].notna().any():
                        ...
                        hoje_datetime = pd.Timestamp(datetime.now().date())
